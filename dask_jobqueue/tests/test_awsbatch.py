...
from dask_jobqueue import AWSBatchCluster

from . import QUEUE_WAIT


def test_header():
    with AWSBatchCluster(
    ...
    with AWSBatchCluster(cores=4, memory="8GB") as cluster:
...
def test_config_name_slurm_takes_custom_config():
    ...
    with dask.config.set({"jobqueue.awsbatch-config-name": conf}):
        with AWSBatchCluster(config_name="awsbatch-config-name") as cluster:
